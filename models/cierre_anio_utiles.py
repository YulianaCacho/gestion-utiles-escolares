...
from .almacen_utiles import GRADO_ESCOLAR_SELECTION
...
class CierreAnioUtiles(models.Model):
    ...
    def action_generar_revision(self):
        for rec in self:
            ...
            if not lineas:
                raise UserError("No se encontraron útiles sobrantes para este año escolar.")
    ...
    def action_confirmar_traslado(self):
        ...
        for rec in self:
            ...
            for linea in rec.linea_ids:
                ...
                if linea.cantidad_a_trasladar > linea.cantidad_sistema:
                    raise UserError(
                        f"No puedes trasladar más de lo que existe según sistema para {linea.product_id.display_name}."
                    )
